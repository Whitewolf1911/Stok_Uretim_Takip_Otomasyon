...
{
    public partial class Frm_AyakKesmeEkle : Form
    {
        public Frm_AyakKesmeEkle()
        {
            InitializeComponent();
        }
        ...
        private void Btn_Kaydet_Click(object sender, EventArgs e)
        {
            try
            {
                if (lookUp_Siparis.EditValue != null && date_BasimTarihi.EditValue != null)
                {
                    ...
                    rapor.ISLEM = "AyakKesme";
                    rapor.URUNTUR = lookUp_Siparis.GetColumnValue("Tur").ToString();
                    db.TBL_RAPOR.Add(rapor);
                    db.SaveChanges();

                    XtraMessageBox.Show("Ayak Kesme Raporu Eklendi", "İşlem Başarılı", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    var deger = db.TBL_SIPARIS.Find(rapor.SIPARISNO);
                    deger.AYAKKESMESAYI += int.Parse(num_IslenenAdet.Value.ToString());
        ...
        private void Frm_AyakKesmeEkle_Load(object sender, EventArgs e)
        {
            try
            {
